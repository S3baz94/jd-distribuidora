      ...
      <div className="max-w-7xl mx-auto px-2.5 sm:px-4 py-2 flex flex-col md:flex-row items-center justify-between gap-2">
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-3">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <div>
              <span className="text-[10px] font-black tracking-widest text-gold-400 uppercase block leading-none">
                ECOSISTEMA DIGITAL CONECTADO
              </span>
              <span className="font-bebas text-sm sm:text-base text-white tracking-wider leading-none">
          ...
          <div className="flex md:hidden items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-950/80 border border-emerald-500/50 text-[10px] font-black text-emerald-400">
          ...
          {tools.map((tool) => {
            ...
            return (
              <Link
                key={tool.id}
                href={tool.href}
                className={`relative px-3 sm:px-3.5 py-1.5 rounded-2xl flex items-center gap-2 transition-all flex-shrink-0 border-2 ${
                  tool.isActive
