...
export const PlantPackingStation: React.FC<PlantPackingStationProps> = ({
  ...
  const routeOrders = React.useMemo(() => {
    if (activeRouteId === "all") {
      return allOrders.filter((o) => o.status !== "cancelled" && o.status !== "delivered");
    }
    return allOrders.filter(
      (o) => o.routeId === currentRoute?.id || currentRoute?.orderIds.includes(o.id)
    );
  }, [allOrders, activeRouteId, currentRoute]);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center border border-emerald-500/30 font-bold flex-shrink-0">
              <ClipboardList className="w-5 h-5" />
            </div>
            <div className="min-w-0 flex-1">
              <span className="text-[10px] font-black uppercase tracking-wider text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20 inline-block">
                ALISTAMIENTO DE PEDIDOS (DESDE APP DE VENTAS)
              </span>
              <h2 className="text-base sm:text-lg font-black text-white truncate">
                Planilla de Desposte & Alistamiento de Pedidos
              </h2>
            </div>
          </div>

          <div className="flex items-center gap-2 w-full sm:w-auto">
            <span className="text-xs text-slate-400 font-bold hidden sm:inline flex-shrink-0">Filtro:</span>
            <select
              value={activeRouteId}
              onChange={(e) => {
              ...
              className="w-full sm:w-auto bg-slate-950 border border-slate-700 text-white font-bold text-xs rounded-xl px-3 py-2 focus:outline-none focus:border-emerald-500 truncate"
            >
              <option value="all">📋 Todos los Pedidos Activos de Ventas</option>
              {routes.map((r) => (
                <option key={r.id} value={r.id}>
                  🚛 Furgón {r.vehiclePlate} • {r.driverName} ({r.name})
                </option>
              ))}
            </select>
          </div>
